...
# Paramiko Expect
#
# Written by Fotis Gimian, modified by LSc
...
class SSHClientInteraction:
    ...
    def expect(self, re_strings=''):
        ...
        while len(re_strings) == 0 or \
                not [re_string for re_string in re_strings if
        ...
        if len(re_strings) != 0:
            found_pattern = [(re_index, re_string) for re_index, re_string in enumerate(re_strings) if
                             re.match('.*\n' + re_string + '$', self.current_output, re.DOTALL)]
